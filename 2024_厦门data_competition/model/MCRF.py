...
'''
    亲测有效,全面上线替代
    aspect valid F1 0.851 -> 0.864   1.6%
    fault valid F1 0.557 -> 0.588   5.6%
    TODO
    1. https://github.com/kajyuuen/pytorch-partial-crf/tree/master   Partial CRF/ Marginal CRF    希望解决漏标注/模糊标注问题
    2. AllenNLP Weighted CRF  https://eraldoluis.github.io/2022/05/10/weighted-crf.html#trellis
 '''
class CRF(nn.Module):
    ...
    def __init__(self, num_tags: int, batch_first: bool = False, label2idx: dict = None) -> None:
        ...
        # self.mask_tran_matrix = torch.tensor(self.get_mask_trans()).cuda()  #部署的时候没有cuda，这里待测试
    ...
    def _compute_score(
        ...
        self.transitions.data = self.transitions.min(self.mask_tran_matrix) #取对应的掩码矩阵值 -100
    ...
    def _compute_normalizer(
        ...
        score = self.start_transitions + emissions[0]   # 动态规划的结构定义很重要，当前时间步以tag j结尾的序列概率
        ...
        for i in range(1, seq_length):
            ...
            # shape: (batch_size, num_tags)
            score = torch.where(mask[i].unsqueeze(1), next_score, score)    #这样做的目的是为了确保在计算分母时，只考虑有效的标签序列，而忽略那些填充的部分。这是在处理变长序列时常见的一种做法，以确保模型的计算正确且有效。
